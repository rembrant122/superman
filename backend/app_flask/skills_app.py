...
from models import AutomatisationBaseModel, ResultRepeatSkill, SkillModel
...
def get_skill_for_repeat():
    with session_scope() as session:
        ...
        if not ready_skill_db:
            return send_message()

        skill = SkillModel.model_validate(ready_skill_db)
...
def get_skill_for_memorize():
    with session_scope() as session:
        ...
        skill_model = SkillModel.model_validate(skill_db)
        return send_message(skill_model)
